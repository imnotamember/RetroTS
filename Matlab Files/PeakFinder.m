...
%Make 'e' or Error = 0
e = 0;
%Make 'R' a default structure
R = struct([]);

% if arguments are less than 2, then make 'Opt' an empty struct
if (nargin < 2) Opt = struct(); end
% Set defaults for:
%PhysFS = 1/0.025; %sampling frequency
%zerophaseoffset = 0.5;  %Fraction of the period that corresponds
    %to a phase of 0
...
% if this is run as a demo, turn quiet off
if (Opt.Demo),
    Opt.Quiet = 0;
else
    pause off
...
%b = fir1(n,Wn,'ftype') returns row vector b containing the n+1 
% coefficients of an order n lowpass FIR filter. This is a Hamming-window
% based, linear-phase filter with normalized cutoff frequency Wn.
% The output filter coefficients, b, are ordered in descending powers of z.
% -Laymen's terms: b = Window-based finite impulse response filter of
%  'FIROrder'+1 amount of coefficients of an order 'FIROrder' lowpass FIR
%  filter. 
...
% if vvec is a character based array, run zglobb on it
if (ischar(vvec)),
    l = zglobb(vvec);
    nl = length(l);
    if (isnumeric(l)),
